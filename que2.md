...
•	`return_type` – тип (класс), на который операторная функция возвращает указатель, для которого выделена память по особому (нестандартным способом). Иногда здесь используют просто `void*`, везде пишут по разному;
